...
모든 그림은 논문 관례로 그려집니다. 세리프 글꼴(Times New Roman → STIX → DejaVu Serif 순서로 있는 것), STIX 수식
글꼴, 네 변 안쪽 눈금, 검은 1.2 pt 테두리, 10⁻²~10³을 벗어나면 ×10ⁿ 수학식 지수 표기, 검은 테두리 범례. 글꼴 크기는
적응형입니다. 8×6인치 기준 크기표를 그림의 실제 크기에 비례해 조절하고(0.6~1.3배), 서브플롯이
n개면 1/√n으로 줄이며, 3D 축은 눈금 라벨이 겹치기 쉬워 0.8배와 눈금 5개 이하로 두고 제목 자리(상단 8%)를 비워 둡니다.
각 모듈 창의 **Plot** 메뉴에서 전체 배율(**Font scale** 50~200%, 단축키 Ctrl +, Ctrl −, Ctrl 0)을 조절하며, 값은 모든
창에 공통이고 저장됩니다. 그림 창마다 있는 **Publication figure…** 버튼(Plot 메뉴의 같은 항목, Ctrl E)은 현재 그림을
8×6인치 안에 맞춘 뒤 논문용 글꼴 크기(제목 24, 축 22, 눈금 20, 범례 18 pt; 서브플롯 수와 Font scale 반영)로
600 dpi PNG/TIFF 또는 PDF/SVG로 저장하므로 창 크기와 무관하게 같은 모양이 나옵니다. 구현은 `geomech/gui/plotstyle.py`이며
패널이 그린 뒤에 스타일을 입히므로 3D·극좌표 축은 글꼴만 바뀌고 컬러바는 눈금을 유지합니다.
