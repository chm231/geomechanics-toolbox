...
if size(center, 2) > 1,
	...
	out(k, :) = min(sqrt(sum(((data-ones(size(data, 1), 1)*center(k, :)).^2)')), sqrt(sum(((-data-ones(size(data, 1), 1)*center(k, :)).^2)')));
    end
else	% 1-D data
    for k = 1:size(center, 1),
    ...
    msgbox([{'distfcm_new.m' 'Error found in line ' num2str(errmsg)} {ex.message}]);
end