function UCS = calUCS2(delta, phi, Pp, Pmud, v, S_g, amatte, Gtc, Gec)
...
Tsig_for_stress=[L1^2 M1^2 N1^2 2*M1*N1 2*N1*L1 2*L1*M1; 
    L2^2 M2^2 N2^2 2*M2*N2 2*N2*L2 2*L2*M2; 
    L3^2 M3^2 N3^2 2*M3*N3 2*N3*L3 2*L3*M3; 
    L2*L3 M2*M3 N2*N3 M2*N3+M3*N2 N2*L3+N3*L2 L2*M3+L3*M2; 
    -(L3*L1) -(M3*M1) -(N3*N1) -(M1*N3+M3*N1) -(N1*L3+N3*L1) -(L1*M3+L3*M1); 
...
stressdevmat=[(sin(-Gpol))^2 (cos(-Gpol)*cos(-Gazi))^2 (cos(-Gpol)*sin(-Gazi))^2; % ¿ø·¡ toolbox
