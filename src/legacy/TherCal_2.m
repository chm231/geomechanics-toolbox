...
function varargout = TherCal_2(varargin)
...
% Last Modified by GUIDE v2.5 05-Dec-2016 06:14:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TherCal_2_OpeningFcn, ...
                   'gui_OutputFcn',  @TherCal_2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function TherCal_2_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = TherCal_2_OutputFcn(hObject, eventdata, handles) 
...
function bdbutton_Callback(hObject, eventdata, handles)
...
if file ~= 0
    ...
    if strcmp(temp{1}{1}, 'J/kg¡ÆC')~=1
        set(handles.c_R, 'String', temp{1}{1});
    end
    if strcmp(temp{2}{1}, 'J/kg¡ÆC')~=1
        set(handles.c_W, 'String', temp{2}{1});
    end
    if strcmp(temp{3}{1}, 'kg/m^3')~=1
        set(handles.Rho_R, 'String', temp{3}{1});
    end
    if strcmp(temp{4}{1}, 'W/m¡ÆC')~=1
        set(handles.K_R, 'String', temp{4}{1});
    end
    if strcmp(temp{5}{1},'¡ÆC')~=1
        T_ro=str2num(cell2mat(temp{5}{1}));
        set(handles.T_RO, 'String', temp{5}{1});
    end
    if strcmp(temp{6}{1}, '¡ÆC')~=1
        T_wo=str2num(cell2mat(temp{6}{1}));
        set(handles.T_WO, 'String', temp{6}{1});
    end
    ...
    if strcmp(temp{1}{1},'m')~=1
        set(handles.L_F, 'String', temp{1}{1});
    end
    if strcmp(temp{2}{1}, 'kg/s')~=1
        set(handles.Q_M, 'String', temp{2}{1});
    end
    N=1; %number of fractures in the Bodvarsson equation case
    if strcmp(temp{3}{1}, '\r\n') ~=1
        N=str2num(cell2mat(temp{3}{1})); %number of fractures in Gringarten and radial cases
        set(handles.N_F, 'String', temp{3}{1});
    end
    if strcmp(temp{4}{1},'m')~=1
        x_E=str2num(cell2mat(temp{4}{1}))/2;
        set(handles.x_e, 'String', temp{4}{1});   
    end
    if strcmp(temp{5}{1},'m')~=1
        set(handles.z_D, 'String', temp{5}{1});
    end
    if strcmp(temp{6}{1},'years')~=1
        set(handles.t_YEAR, 'String', temp{6}{1});
    end
...
% --- Executes on key press with focus on Cal_button and none of its controls.
function Cal_button_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to Cal_button (see GCBO)
