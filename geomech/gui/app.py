...
from geomech.gui.module_window import ModuleWindow, apply_plot_settings, settings  # noqa: F401  (settings re-exported)
...
# (label, factory) - order follows Simulator_int.mlapp
MODULES = [
...
class Launcher(QMainWindow):
    def __init__(self):
        ...
        for i, (name, factory) in enumerate(MODULES):
            ...
            btn.clicked.connect(lambda _=False, f=factory, n=name: self.open_module(f, n))
            grid.addWidget(btn, i // 2, i % 2)
        v.addStretch(1)

    def open_module(self, factory, name: str = "") -> ModuleWindow:
        """Open a module in its own window (menu bar with the plot options around the panel)."""
        win = ModuleWindow(factory(), name)
        win.show()
